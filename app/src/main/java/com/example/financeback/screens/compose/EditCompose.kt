...
@Composable
fun EditScreen(modifier: Modifier = Modifier, income: IncomeInfo, userID: Int, navController: NavController) {
    ...
    var number by remember { mutableStateOf(String.format("%.2f", editIncome.value).replace(".", "").replace(",", "")) }
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        Column(
            ...
            Button(onClick = { editIncome.value = NumberFormatter().doubleFormatter(number)
                missingParams = editIncome.missingParam().isNotEmpty()
                if(!missingParams)
                    incomeEdited = Income().editIncome(context, userID, editIncome) }) {
                Text(text = "Editar")
            }
        }
        ...
        if(incomeEdited) {
            AlertDialog(onDismissRequest = { incomeEdited = false },
                ...
                confirmButton = { 
                    Button(onClick = { incomeEdited = false
